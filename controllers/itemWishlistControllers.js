...
const getItemsInWishlist = async (req, res, next) => {
  try {
    ...
    const response = await itemWishlistServices.getItemsInWishlist(
      req.params.id
    );
...
const getItemsInWishlist_WishlistAndItemId = async (req, res, next) => {
  try {
    const response =
      await itemWishlistServices.getItemsInWishlist_WishlistAndItemId(
        req.params.itemId,
...
const addItemsInWishlist = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(req.body.wishlistId)) {
      const wishlist = await wishlistServices.getWishlistById(
        req.body.wishlistId
      );
      if (!wishlist) {
        throw { message: "No wishlist found" };
...
const updateItemsInWishlist = async (req, res, next) => {
  try {
    ...
    const itemsInWishlist = await itemWishlistServices.getItemsInWishlist(
      req.params.id
    );
    ...
    if (req?.body?.wishlistId) {
      const newWishlist = await wishlistServices.getWishlistById(
        req.body.wishlistId
      );
      if (!newWishlist) {
        throw { message: "No wishlist found" };
    ...
    const response = await itemWishlistServices.updateItemsInWishlist(
      req.params.id,
      objectResponse
    );
    res.json(response);
...
const deleteItemsInWishlist = async (req, res, next) => {
  try {
    ...
    const response = await itemWishlistServices.deleteItemsInWishlist(
      req.params.id
    );
...
export default {
  getItemsInWishlists,
  getItemsInWishlist,
  getItemsInWishlist_WishlistAndItemId,
  addItemsInWishlist,
  updateItemsInWishlist,
  deleteItemsInWishlist,
};
